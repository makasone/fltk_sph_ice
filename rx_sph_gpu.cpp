 ...
 * @param[in] d2h デバイスからホストへのデータ転送フラグ
 * @return ホストメモリ上のデータ
 */
 ...
 * @param[in] minp グリッドの最小座標
 * @param[in] d グリッド幅
 * @param[in] fIceCheck 氷かどうかのフラグ　0.0f→水　それ以外→氷
 * @param[out] hF 陰関数値(nx×ny×nzの配列)
 */
 ...
 * @param[in] minp グリッドの最小座標
 * @param[in] d グリッド幅
 * @param[in] fIceCheck 氷かどうかの判定配列　0.0f→水　それ以外→氷
 * @param[out] dF 陰関数値(nx×ny×nzの配列)
 */
...
double rxSPH_GPU::CalColorFieldSolid(double x, double y, double z, float* fIceCheck)
{
	...
	//GetNNは，posを中心に，粒子配列m_hPosを基にした近傍情報を，影響半径hで，neに入れるという処理
	//各グリッドにどの粒子が入っているのかを知りたい．
	//
...
//		RXREAL r = length(pos-pos1);
		...
		//テスト
//		if(r < 0.95*h){
//			c += m_params.Mass*m_fpW(r, 0.95*h, m_fAw);	//h，m_fAwは定数
//		}

		if( fIceCheck[j] < 0.0f ) continue;		//氷でないならメッシュを作らない
		c += m_params.Mass*m_fpW(r, h, m_fAw);	//h，m_fAwは定数
	}
