 ...
 */
void rxPBDSPH::Initialize(const rxSPHEnviroment &env)
{
	// MARK:Initialize
	...
	// 境界パーティクル生成
	// 修正：：ここでパーティクルの間隔をいじる	def::0.75
	m_uNumBParticles = m_pBoundary->GenerateParticlesOnSurf(1.2*m_fParticleRadius, &m_hPosB);
#endif
...
bool rxPBDSPH::Update(RXREAL dt, int step)
{
	...
	while(((dens_var > m_fEta) || (iter < m_iMinIterations)) && (iter < m_iMaxIterations)){
		...
		// 衝突処理
		integrate(m_hPredictPos, m_hPredictVel, m_hDens, m_hFrc, m_hPredictPos, m_hPredictVel, dt);
 ...
 */
void rxPBDSPH::SetColorVBO(int type)
{
	switch(type){
	case RX_DENSITY:
...
void rxPBDSPH::calScalingFactor(const RXREAL *ppos, RXREAL *pdens, RXREAL *pscl, RXREAL h, RXREAL dt)
{
	...
	for(uint i = 0; i < m_uNumBParticles; ++i){
		...
		for(vector<rxNeigh>::iterator itr = bneigh.begin() ; itr != bneigh.end(); ++itr){
			...
			Vec3 dpb = (m_fRestDens*m_hVolB[j]/m_fMass)*m_fpGW(r, h, m_fAg, rij)/r0;
...
void rxPBDSPH::calPositionCorrection(const RXREAL *ppos, const RXREAL *pscl, RXREAL *pdp, RXREAL h, RXREAL dt)
{
	...
	for(uint i = 0; i < m_uNumParticles; ++i){
		...
		m_hDebugVec[DIM*i+0] = dpbij[0];
		m_hDebugVec[DIM*i+1] = dpbij[1];
		m_hDebugVec[DIM*i+2] = dpbij[2];
 ...
 */
void rxPBDSPH::DetectSurfaceParticles(void)
{
	RXREAL h = m_fEffectiveRadius;
	RXREAL r = m_fParticleRadius;
 ...
 */
double rxPBDSPH::CalDistToNormalizedMassCenter(const int i)
{
	Vec3 sum_pos(0.0);
	double sum_mass = 0.0;
 ...
 */
uint* rxPBDSPH::GetArraySurf(void)
{
	return m_hSurf;
}
 ...
 * @return 見つかったパーティクル数
 */
int rxPBDSPH::GetSurfaceParticles(const Vec3 pos, RXREAL h, vector<rxSurfaceParticle> &sps)
{
	int num = 0;
	sps.clear();
 ...
 * 密度分布から法線を計算
 */
void rxPBDSPH::CalNormalFromDensity(void)
{
	RXREAL h = m_fEffectiveRadius;
 ...
 * @return 格納ポリゴン数
 */
int rxPBDSPH::GetPolygonsInCell(int gi, int gj, int gk, vector<int> &polys)
{
 ...
 * 固体障害物の描画
 */
void rxPBDSPH::DrawObstacles(void)
{
	for(vector<rxSolid*>::iterator i = m_vSolids.begin(); i != m_vSolids.end(); ++i){
		(*i)->Draw();
	}
 ...
 * @param[in] disp 移動量
 */
void rxPBDSPH::MoveSphereObstacle(int b, Vec3 disp)
{
 ...
 * @param[in] b 物体番号
 */
Vec3 rxPBDSPH::GetSphereObstaclePos(int b)
{
 ...
 */
void rxPBDSPH::CalAnisotropicKernel(void)
{
