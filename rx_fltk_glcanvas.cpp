...
//	RXTIMER("InitHT");
/*
	InitSM(m_Scene);		// SM法初期化
//	RXTIMER("InitSM");
	InitICE(m_Scene);		// 氷構造の初期化
//	RXTIMER("InitICE");
	InitSM_Layer(m_Scene);	// 階層構造の作成
//	RXTIMER("InitSMLayer");
	InitICE_Layer(m_Scene);
//	RXTIMER("InitICELayer");
*/
	InitTetra();
//	RXTIMER("InitTetra");
	InitCluster();
...
//		StepSM(m_fDt);						//SM法　粒子位置のフィードバック，計算，線形補間
	...
	else if( m_bMode == MODE_ICE )
	{
		...
		//四面体ベース
		//RXTIMER("Param_start");
		//StepICE_CalcParametor(m_fDt);		//温度・熱を基にした各パラメータの計算
		//RXTIMER("Param_end");

		//RXTIMER("Freeze_start");
		//StepICE_Freeze();					//凝固処理
		//RXTIMER("Freeze_end");
		...
		//粒子ベース
		StepSolid_Melt(m_fDt);
		StepSolid_Freeze(m_fDt);

		//RXTIMER("Cluster_start");
...
//		cout << __FUNCTION__ << "check start" <<endl;
		//追加：粒子数をチェックして，各情報を更新
		UpdateInfo();
//		cout << __FUNCTION__ << "check end" <<endl;
	}
}
 ...
 * @param[in] fn_scene シーン記述ファイル名
 */
void rxFlGLWindow::InitSM(rxSPHConfig &sph_scene)
{	cout << __FUNCTION__ << endl;
	//パーティクルの初期位置情報を登録
	//m_pPS->SetParticlesToCell(	m_pPS->GetParticle(), 
	//							m_pPS->GetNumParticles(), 
	//							((RXSPH*)m_pPS)->GetEffectiveRadius()/1.25
	//						);													//影響半径の半分の領域で粒子を取得
	//vector<vector<rxNeigh>>& neights = ((RXSPH*)m_pPS)->GetNeights();			//近傍粒子を取得

	//変数の初期化
	for( vector<Ice_SM*>::iterator it = m_sm_connects.begin(); it != m_sm_connects.end(); ++it )
	{
		if(*it) delete *it;
	}

	for( vector<Ice_SM*>::iterator it = m_sm_calcs.begin(); it != m_sm_calcs.end(); ++it )
	{
		if(*it) delete *it;
	}
	...
	//tetgenを利用した四面体リストの取得
	MakeTetrahedra();						//tetgenを利用した四面体作成
//	Load_ELE_File("test.1.ele");			//eleファイルを読み込み，リストを作成
//	Load_ELE_File("sph_pos_3_3.1.ele");
//	Load_ELE_File("sph_pos_5_5.1.ele");
//	Load_ELE_File("test_out_ele.1.ele");
//	Save_POLY_File();						//polyファイル作成
											
	m_iClusteresNum = 0;					//クラスタ数

	for(unsigned i = 0; i < m_vviTetraList.size(); i++)
	{
		MakeCluster(m_vviTetraList[i]);		//四面体リストを元に，SM法のクラスタ作成
	}
}

/*!
 * クラスタへ粒子の追加　単純にaddVertexするとうまくいかないので，新しく作ったものと差し替える
 * @param[in] pIndx	追加する粒子
 * @param[in] cIndx 追加されるクラスタ
 */
void rxFlGLWindow::AddVertexToCluster(int pIndx, int cIndx)
{
	RXREAL *p = m_pPS->GetArrayVBO(rxParticleSystemBase::RX_POSITION);
	rxSPHEnviroment sph_env = m_Scene.GetSphEnv();

	//差し替えるクラスタの作成
	Ice_SM *newCluster_Connect = new Ice_SM(cIndx);
	newCluster_Connect->SetSimulationSpace(-sph_env.boundary_ext, sph_env.boundary_ext);
	newCluster_Connect->SetTimeStep(sph_env.smTimeStep);
	newCluster_Connect->SetCollisionFunc(0);
	newCluster_Connect->SetStiffness(1.0, 0.0);

	Ice_SM *newCluster_Calc = new Ice_SM(cIndx);
	newCluster_Calc->SetSimulationSpace(-sph_env.boundary_ext, sph_env.boundary_ext);
	newCluster_Calc->SetTimeStep(sph_env.smTimeStep);
	newCluster_Calc->SetCollisionFunc(0);
	newCluster_Calc->SetStiffness(1.0, 0.0);

	//新しく作成するクラスタの粒子
	vector<int> pList;
	for(int i = 0; i < m_sm_connects[cIndx]->GetNumVertices(); i++)
	{
		int ipIndx = m_sm_connects[cIndx]->GetParticleIndx(i);
		pList.push_back(ipIndx);
	}

	pList.push_back(pIndx);

	//古い情報の削除
	DeleteIceInfoCluster(cIndx);
	...
	for( unsigned i = 0; i < pList.size(); i++ )
	{
		...
		newCluster_Connect->AddVertex( Vec3( p[ipIndx*4+0], p[ipIndx*4+1], p[ipIndx*4+2] ), 1.0, ipIndx );
		newCluster_Connect->SetAlphas(i, 1.0);
		newCluster_Connect->SetBetas (i, 0.0);

		newCluster_Calc->AddVertex( Vec3( p[ipIndx*4+0], p[ipIndx*4+1], p[ipIndx*4+2] ), 1.0, ipIndx );
		newCluster_Calc->SetAlphas(i, 1.0);
		newCluster_Calc->SetBetas (i, 0.0);
		newCluster_Calc->SetLayer (i, 0);
	}

	////クラスタの入れ替え
	Ice_SM *tempSM = m_sm_connects[cIndx];
	m_sm_connects[cIndx] = newCluster_Connect;								
	delete tempSM;

	tempSM = m_sm_calcs[cIndx];
	m_sm_calcs[cIndx] = newCluster_Calc;								
	delete tempSM;

	//新しい情報の登録
	MakeClusterInfo_Connect(cIndx);					//接続クラスタの情報登録
	m_ice->SetNeighborCluster(cIndx, m_iLayer);		//近傍クラスタの情報登録
	MakeClusterCalc(cIndx);							//計算クラスタ（完成）の作成
	MakeClusterInfo_Calc(cIndx);					//計算クラスタの情報登録
	MakeCalcToConnectInfo(cIndx);					//計算クラスタ→接続クラスタの情報登録

	//デバッグ
	cout << __FUNCTION__ << " debug pIndx = " << pIndx << " cIndx = " << cIndx << endl;
	//m_sm_connects[cIndx]->DebugIndx();
	//m_ice->DebugCtoP_Connect(cIndx);

	//m_sm_calcs[cIndx]->DebugIndx();
	//m_ice->DebugCtoP_Calc(cIndx);
	//m_ice->DebugNeighborCluster(cIndx);
	//m_ice->DebugCalcToConnect(cIndx);
}

/*!
 * クラスタ作成　接続クラスタ，計算クラスタ（初期）の両方
 * @param[in] pList	クラスタを構成する粒子リスト
 */
void rxFlGLWindow::MakeCluster(vector<int> pList)
{//	cout << __FUNCTION__ << endl;
	...
	//接続情報クラスタ
	m_sm_connects.push_back(new Ice_SM(m_iClusteresNum));
	m_sm_connects[m_iClusteresNum]->SetSimulationSpace(-sph_env.boundary_ext, sph_env.boundary_ext);
	m_sm_connects[m_iClusteresNum]->SetTimeStep(sph_env.smTimeStep);
	m_sm_connects[m_iClusteresNum]->SetCollisionFunc(0);
	m_sm_connects[m_iClusteresNum]->SetStiffness(1.0, 0.0);

	for(unsigned i = 0; i < pList.size(); i++)
	{
		int pIndx = pList[i];

		m_sm_connects[m_iClusteresNum]->AddVertex( Vec3( p[pIndx*4+0], p[pIndx*4+1], p[pIndx*4+2] ), 1.0, pIndx );
		m_sm_connects[m_iClusteresNum]->SetAlphas(i, 1.0);
		m_sm_connects[m_iClusteresNum]->SetBetas (i, 0.0);
	}

	//計算処理クラスタ
	m_sm_calcs.push_back(new Ice_SM(m_iClusteresNum));
	m_sm_calcs[m_iClusteresNum]->SetSimulationSpace(-sph_env.boundary_ext, sph_env.boundary_ext);
	m_sm_calcs[m_iClusteresNum]->SetTimeStep(sph_env.smTimeStep);
	m_sm_calcs[m_iClusteresNum]->SetCollisionFunc(0);
	m_sm_calcs[m_iClusteresNum]->SetStiffness(1.0, 0.0);

	//これだとできる
	//つまり，SM法でなく数値に問題がある
	//m_sm_calcs[cIndx]->AddVertex(Vec3( -0.1, 0.05, -0.1 ), 0.01, 0);
	//m_sm_calcs[cIndx]->AddVertex(Vec3(  0.1, 0.05, -0.1 ), 0.01, 1);
	//m_sm_calcs[cIndx]->AddVertex(Vec3( -0.1, 0.25, -0.1 ), 0.01, 2);
	//m_sm_calcs[cIndx]->AddVertex(Vec3( -0.1, 0.05,  0.1 ), 0.01, 3);

	//これ以下だとできない
	//ある値以下になると，うまく行列が求められないみたい
	//m_sm_calcs[cIndx]->AddVertex(Vec3( -0.1, 0.05, -0.1 )*0.5, 1.0, 0);
	//m_sm_calcs[cIndx]->AddVertex(Vec3(  0.1, 0.05, -0.1 )*0.5, 1.0, 1);
	//m_sm_calcs[cIndx]->AddVertex(Vec3( -0.1, 0.25, -0.1 )*0.5, 1.0, 2);
	//m_sm_calcs[cIndx]->AddVertex(Vec3( -0.1, 0.05,  0.1 )*0.5, 1.0, 3);
	//massも関係しているので注意．上記だと，mass!=1.0のとき．
	
	for( unsigned i = 0; i < pList.size(); i++ )
	{
		int pIndx = pList[i];

		m_sm_calcs[m_iClusteresNum]->AddVertex( Vec3( p[pIndx*4+0], p[pIndx*4+1], p[pIndx*4+2] ), 1.0, pIndx );
		m_sm_calcs[m_iClusteresNum]->SetAlphas(i, 1.0);
		m_sm_calcs[m_iClusteresNum]->SetBetas (i, 0.0);
		m_sm_calcs[m_iClusteresNum]->SetLayer(i, 0);
	}

	m_iClusteresNum++;
	if(m_ice == 0)	return;
	m_ice->SetClusterNum(m_iClusteresNum);
}

/*!
 * SM法の初期化　近傍クラスタから粒子の追加　必ずInitICEの後に呼ぶ
 * @param[in] fn_scene シーン記述ファイル名
 */
void rxFlGLWindow::InitSM_Layer(rxSPHConfig &sph_scene)
{	cout << "InitSM_Layer" << endl;
	
	//近傍クラスタを探索し，周囲の粒子を追加する
	for(int i = 0; i < m_iClusteresNum; i++)
	{
		MakeClusterCalc(i);
	}
...
/*!
 * クラスタ作成　計算クラスタ（完成）の作成
 * @param[in] cIndx 作成する計算クラスタ番号
 */
void rxFlGLWindow::MakeClusterCalc(int cIndx)
{//	cout << __FUNCTION__ << endl;
	RXREAL *p = m_pPS->GetArrayVBO(rxParticleSystemBase::RX_POSITION);
		
	for(int i = 0; i < m_ice->GetNCNum(cIndx); i++)
	{
		int icIndx = m_ice->GetNeighborCluster(cIndx, i)[0];
		int layer  = m_ice->GetNeighborCluster(cIndx, i)[1];

		for(int j = 0; j < m_ice->GetCtoPIndx_Connect(icIndx); j++ )
		{
			int pIndx = m_ice->GetCtoP_Connect(icIndx, j);

			if(pIndx == -1){							continue;	}
			if(m_sm_calcs[cIndx]->CheckIndx(pIndx)){	continue;	}

			m_sm_calcs[cIndx]->AddVertex( Vec3( p[pIndx*4+0], p[pIndx*4+1], p[pIndx*4+2] ), 1.0, pIndx );
			m_sm_calcs[cIndx]->SetAlphas(m_sm_calcs[cIndx]->GetNumVertices()-1, 1.0);
			m_sm_calcs[cIndx]->SetBetas (m_sm_calcs[cIndx]->GetNumVertices()-1, 1.0);
			m_sm_calcs[cIndx]->SetLayer (m_sm_calcs[cIndx]->GetNumVertices()-1, layer);
		}
	}
}
 ...
 * @param[in] dt タイムステップ幅
 */
void rxFlGLWindow::StepSM(double dt)
{//	cout << __FUNCTION__ << endl;
	if(m_bPause){	return;}
	...
	//for( int i = 0; i < m_pPS->GetNumParticles(); i++ )
	//{
	//	if( m_fIntrps.size() <= (unsigned)i ) continue;

	//	for( int j = 0; j < m_ice->GetPToCListNum(i); j++ )
	//	{
	//		int* coSet = m_ice->GetPtoC_Connect(i, j);
	//		int cIndx = coSet[0];
	//		int oIndx = coSet[1];
	//		m_sm_connects[cIndx]->SetCurrentPos	(oIndx, Vec3( p[i*4+0], p[i*4+1], p[i*4+2] ));
	//		m_sm_connects[cIndx]->SetVelocity	(oIndx, Vec3( v[i*4+0], v[i*4+1], v[i*4+2] ));	//速度は未更新
	//	}
	//}
...
#pragma omp parallel	//影響大
{
#pragma omp for private(j, pIndx, cIndx, oIndx)
	//計算用クラスタの処理
	for(int i = 0; i < m_sm_calcs.size(); i++)
	{
		//計算処理クラスタの数値更新
		for(j = 0; j < m_ice->GetCtoPNum_Calc(i); j++)
		{
			cIndx = m_ice->GetCalcToConnect(i, j)[0];										//どの氷塊からの粒子
			oIndx = m_ice->GetCalcToConnect(i, j)[1];										//氷塊内で何番目の粒子か
			pIndx = m_sm_connects[cIndx]->GetParticleIndx(oIndx);
			
			//エラーチェック
			//if( abs(pIndx) > m_pPS->GetNumParticles() )
			//{
			//	cout << "i = " << i << " j = " << j << " cIndx = " << cIndx << " oIndx = " << oIndx << " pIndx = " << pIndx << endl;
			//	m_sm_connects[cIndx]->DebugIndx();
			//	m_sm_calcs[cIndx]->DebugIndx();

			//	m_sm_connects[i]->DebugIndx();
			//	m_sm_calcs[i]->DebugIndx();

			//	m_ice->DebugCalcToConnect(i);
			//}

			m_sm_calcs[i]->SetCurrentPos	( j, Vec3( p[pIndx*4+0], p[pIndx*4+1], p[pIndx*4+2] ) );	//これをなくすと，粒子の関連を保ちつつ位置が変わる．
//			m_sm_calcs[i]->SetOriginalPos	( j, m_sm_connects[cIndx]->GetOriginalPos(oIndx) );			//これをなくすと追加がうまくいく笑
//			m_sm_calcs[i]->SetGoalPos		( j, m_sm_connects[cIndx]->GetGoalPos(oIndx) );
//			m_sm_calcs[i]->SetNewPos		( j, m_sm_connects[cIndx]->GetNewPos(oIndx) );
			m_sm_calcs[i]->SetVelocity		( j, Vec3( v[pIndx*4+0], v[pIndx*4+1], v[pIndx*4+2] ) );	//速度は未更新	これを更新すると，変になる
//			m_sm_calcs[i]->parames[j].alpha = m_sm_connects[cIndx]->parames[oIndx].alpha;
		}
		...
		//位置，速度のフィードバック
		for(j = 0; j < m_sm_connects[i]->GetNumVertices(); j++)
		{
			m_sm_connects[i]->SetCurrentPos	( j, m_sm_calcs[i]->GetVertexPos(j) );			//使用すると，リング数を上げたときに発散することがある
//			m_sm_connects[i]->SetOriginalPos( j, m_sm_calcs[i]->GetOriginalPos(j) );
//			m_sm_connects[i]->SetGoalPos	( j, m_sm_calcs[i]->GetGoalPos(j) );
//			m_sm_connects[i]->SetOriginalPos( j, m_sm_calcs[i]->GetOriginalPos(j) );		//これをなくすと追加がうまくいく笑
//			m_sm_connects[i]->SetNewPos		( j, m_sm_calcs[i]->GetNewPos(j) );
			m_sm_connects[i]->SetVelocity	( j, m_sm_calcs[i]->GetVertexVel(j) );
		}
	}
}//#pragma omp parallel

	StepPS(dt);	//テスト　粒子法の運動
...
//#pragma omp parallel	//ほぼ影響なし→変数を用意していない！
//{
//#pragma omp for
	//線形補間
	for( int i = 0; i < m_pPS->GetNumParticles(); i++ )
	{
		//if( m_fIntrps.size() <= (unsigned)i ){	continue;}			//存在しない場合は飛ばす　四面体ベースのみ
		if(m_ice->GetPtoCNum(i) == 0){	continue;	}					//クラスタに所属していない場合は戻る
		...
		for( int j = 0; j < m_ice->GetPtoCIndx_Connect(i); j++ )
		{
			...
			int oIndx = coSet[1];
			if(cIndx == -1 || oIndx == -1){	continue;	}
			pos += m_sm_connects[cIndx]->GetVertexPos(oIndx);
			vel += m_sm_connects[cIndx]->GetVertexVel(oIndx);
			shapeNum += 1.0;
		}
		...
		//どのクラスタにも含まれていない氷粒子の場合，運動はSPH法に従う　TODO不完全
		else if( shapeNum == 0 )
		{
...
//	RXTIMER("SetNeighborTetra Start");
 ...
 * @param[in] fn_scene シーン記述ファイル名
 */
void rxFlGLWindow::InitICE(rxSPHConfig &sph_scene)
{	cout << "InitICE" << endl;
//	RXTIMER("InitICE Start");

	rxSPHEnviroment sph_env = sph_scene.GetSphEnv();
	m_iLayer = sph_env.layer;

	//初期化
	m_ice = new IceStructure(1500, 3500);												//最大粒子数　最大クラスタ数　これ以上にメモリを使おうとすると動かなくなる
	m_ice->SetParticleNum(ICENUM);														//現在の粒子数の登録
	m_ice->SetClusterNum(m_iClusteresNum);												//現在のクラスタ数の登録
	
//	RXTIMER("InitICE Init");

	//カウント　粒子が接続クラスタに所属する個数
	int pIndx = 0;

	//TODO::デッドロックを避けてcriticalに並列化すると余計に遅くなってしまう．
	//		リダクションになるよう構造を変える，for文を複数に分ける，をしたほうがいい
	for(int i = 0; i < m_vviTetraList.size(); i++)
	{
		for(int j = 0; j < m_vviTetraList[i].size(); j++)
		{
			pIndx = m_vviTetraList[i][j];
			m_ice->CountPtoC_Connect(pIndx);				//粒子が接続クラスタに所属する個数のカウント
			m_ice->CountCtoP_Connect(i);					//接続クラスタが粒子を含む個数のカウント
		}
	}

//	RXTIMER("InitICE Count");

	m_ice->InitStateConnect();								//接続情報のサイズ確保

	//粒子が所属しているクラスタ数を持つ配列のコピー
	int *PtoCNum = new int[ICENUM];

	for(int i = 0; i < ICENUM; i++)
	{
		PtoCNum[i] = m_ice->GetPtoCNum_Connect(i);
	}

//	RXTIMER("InitICE Copy");

	//接続クラスタデータ登録
	for( int i = 0; i < m_vviTetraList.size(); i++ )
	{
		int* pCountList = new int[m_vviTetraList[i].size()];

		for(int j = 0; j < m_vviTetraList[i].size(); j++)
		{
			pIndx = m_vviTetraList[i][j];
			pCountList[j] = m_ice->GetPtoCNum_Connect(pIndx)-PtoCNum[pIndx];	//粒子が所属する何番目のクラスタなのかを求める
			PtoCNum[pIndx]--;
		}

		MakeClusterInfo_Connect(i, m_vviTetraList[i], pCountList);
		delete[] pCountList;
	}
...
//	RXTIMER("InitICE Set");
	...
	for( int i = 0; i < m_vviTetraList.size(); i++ )
	{
		m_ice->SetNeighborCluster(i, m_iLayer);
	}

//	RXTIMER("InitICE Neighbor");

}

/*!
 * クラスタ情報作成　接続クラスタ　並列処理用
 * @param[in] cIndx クラスタ番号
 * @param[in] pList	クラスタを構成する粒子リスト
 * @param[in] pCountList 粒子が属するクラスタの順序番号　pListの粒子毎に用意
 */
void rxFlGLWindow::MakeClusterInfo_Connect(int cIndx, const vector<int>& pIndxList, int *pCountList)
{//	cout << "MakeClusterInfo_Connect" << endl;
	for(int i = 0; i < m_ice->GetCtoPNum_Connect(cIndx); i++)
	{
		//粒子が何番クラスタに所属しているかを記録
		m_ice->SetPtoC_Connect(pIndxList[i], pCountList[i], cIndx, i);	//接続クラスタ
	}

	m_ice->SetCtoP_Connect(cIndx, pIndxList);							//クラスタがどの粒子を含んでいるか記録
}

/*!
 * クラスタ情報作成　接続クラスタ　クラスタ番号のみで作成
 * @param[in] cIndx クラスタ番号
 */
void rxFlGLWindow::MakeClusterInfo_Connect(int cIndx)
{//	cout << __FUNCTION__ << endl;

	const vector<int>& pIndxList = m_sm_connects[cIndx]->GetVertexIndxList();
	int* pCountList = new int[pIndxList.size()];								//粒子が所属している何番目の接続クラスタか

	//格納場所の探索　－１を探索し上書き
	for(unsigned i = 0; i < pIndxList.size(); i++)
	{
		int pIndx = pIndxList[i];

		for(int j = 0; j < m_pPS->GetMaxParticles()/5/2/4; j++)
		{
			int* coSet = m_ice->GetPtoC_Connect(pIndx, j);

			if(coSet[0] != -1 || coSet[1] != -1){	continue;	}

			if(j >= m_ice->GetPtoCIndx_Connect(pIndx))
			{	m_ice->SetPtoCIndx_Connect(pIndx, j+1);		}				//現在のIndxより大きいなら更新
						
			pCountList[i] = j;	break;
		}
	}
	...
	for(int i = 0; i < pIndxList.size(); i++)
	{
		...
		m_ice->CountPtoC_Connect(pIndx);									//粒子が接続クラスタに所属する個数のカウント
		m_ice->CountCtoP_Connect(cIndx);									//接続クラスタが粒子を含む個数のカウント
	}

	m_ice->SetCtoPIndx_Connect(cIndx, m_ice->GetCtoPNum_Connect(cIndx));	//Indxの登録
	
	for(int i = 0; i < m_ice->GetCtoPNum_Connect(cIndx); i++)
	{		
		m_ice->SetPtoC_Connect(pIndxList[i], pCountList[i], cIndx, i);		//粒子が何番クラスタに所属しているかを記録
	}
...
/*!
 * クラスタ情報作成　計算クラスタ　並列処理用
 * @param[in] cIndx クラスタ番号
 * @param[in] pList	クラスタを構成する粒子リスト
 * @param[in] pCountList 粒子が属するクラスタは順序番号　pListの粒子毎に用意
 */
void rxFlGLWindow::MakeClusterInfo_Calc(int cIndx, const vector<int>& pIndxList, int* pCountList, int* pLayerList)
{//	cout << __FUNCTION__ << endl;
	for(int i = 0; i < m_sm_calcs[cIndx]->GetNumVertices(); i++)
	{
		//粒子が何番クラスタに所属しているかを記録
		m_ice->SetPtoC_Calc(pIndxList[i], pCountList[i], cIndx, i, pLayerList[i]);		//計算クラスタ
	}

	m_ice->SetCtoP_Calc(cIndx, pIndxList, pLayerList);								//クラスタがどの粒子を含んでいるか記録
}
 ...
 * @param[in] cIndx クラスタ番号
 */
void rxFlGLWindow::MakeClusterInfo_Calc(int cIndx)
{//	cout << __FUNCTION__ << endl;

	const vector<int>& pIndxList = m_sm_calcs[cIndx]->GetVertexIndxList();
	int* pCountList = new int[pIndxList.size()];								//粒子が所属している接続クラスタ番号
	int* pLayerList = new int[pIndxList.size()];								//粒子の所属レイヤー

//	cout << __FUNCTION__ << " pIndxList.size() = " << pIndxList.size() << endl;
	...
	for(unsigned i = 0; i < pIndxList.size(); i++)
	{
		...
		for(int j = 0; j < m_pPS->GetMaxParticles()/5/2; j++)
		{
			int* coSet = m_ice->GetPtoC_Calc(pIndx, j);

			if(coSet[0] != -1 || coSet[1] != -1){	continue;	}

			if(j >= m_ice->GetPtoCIndx_Calc(pIndx))
			{
				m_ice->SetPtoCIndx_Calc(pIndx, j+1);					//現在のIndxより大きいなら更新
			}

			pCountList[i] = j;
			pLayerList[i] = m_sm_calcs[cIndx]->GetLayer(i);				//別にここでなくともいい
			break;
		}
	}
	...
	for(int i = 0; i < pIndxList.size(); i++)
	{
		...
		m_ice->CountPtoC_Calc(pIndx);									//粒子が接続クラスタに所属する個数のカウント
		m_ice->CountCtoP_Calc(cIndx);									//接続クラスタが粒子を含む個数のカウント
	}

	m_ice->SetCtoPIndx_Calc(cIndx, m_ice->GetCtoPNum_Calc(cIndx));		//Indxの登録

	for(int i = 0; i < m_sm_calcs[cIndx]->GetNumVertices(); i++)
	{
		m_ice->SetPtoC_Calc(pIndxList[i], pCountList[i], cIndx, i, pLayerList[i]);		//粒子が何番クラスタに所属しているかを記録
	}
 ...
 * @param[in] cIndx クラスタ番号
 */
void rxFlGLWindow::MakeCalcToConnectInfo(int cIndx)
{//	cout << __FUNCTION__ << endl;

	//接続クラスタを基に初期情報の登録
	for(int i = 0; i < m_sm_connects[cIndx]->GetNumVertices(); i++)
	{
		m_ice->SetCalcToConnect(cIndx, i, cIndx, i);		//登録
	}

	//近傍クラスタを基に関連情報の登録
	int kpIndx = 0;
	int icIndx = 0;
	int ioIndx = m_sm_connects[cIndx]->GetNumVertices();
	int icooIndx = 0;	//COnnect Ordr Indx

	for(int i = 0; i < m_ice->GetNCNum(cIndx); i++)
	{
		icIndx = m_ice->GetNeighborCluster(cIndx, i)[0];
		icooIndx = -1;

		for(int j = 0; j < m_ice->GetCtoPIndx_Connect(icIndx); j++)
		{
			kpIndx = m_ice->GetCtoP_Connect(icIndx, j);
			if(kpIndx == -1){	continue;	}
			icooIndx++;
			if(m_sm_calcs[cIndx]->GetParticleIndx(ioIndx) != kpIndx){ continue;	}
			m_ice->SetCalcToConnect(cIndx, ioIndx, icIndx, icooIndx);					//登録
			ioIndx++;
		}
	}
}
...
/*!
 * 氷構造の初期化　周辺クラスタのデータ作成
 * @param[in] fn_scene シーン記述ファイル名
 */
void rxFlGLWindow::InitICE_Layer(rxSPHConfig &sph_scene)
{	cout << "InitICE_Layer" << endl;
	//RXTIMER("InitICE_Layr Start");

	//カウント　粒子が計算クラスタに所属する個数
	for(int i = 0; i < m_iClusteresNum; i++)
	{
		for(int j = 0; j < m_sm_calcs[i]->GetNumVertices(); j++)
		{
			int pIndx = m_sm_calcs[i]->GetParticleIndx(j);
			m_ice->CountPtoC_Calc(pIndx);				//粒子が接続クラスタに所属する個数のカウント
			m_ice->CountCtoP_Calc(i);					//接続クラスタが粒子を含む個数のカウント
		}
	}

	//RXTIMER("InitICE_Layr Count");		
//	cout << __FUNCTION__ << " count" << endl;

	m_ice->InitStateCalc();								//計算情報のサイズ確保

	//粒子が所属しているクラスタ数を持つ配列のコピー
	int *PtoCNum = new int[ICENUM];
	for(int i = 0; i < ICENUM; i++)
	{
		PtoCNum[i] = m_ice->GetPtoCNum_Calc(i);
	}

	//RXTIMER("InitICE_Layr Copy");
//	cout << __FUNCTION__ << " save" << endl;

	int *pCountList;	//ポインタの場合，privateは守られるのか？またdelete[]はどうなる？
	int *pLayerList;
	int pIndx = 0;
	int cIndx = 0;
	int j = 0;
	...
	for( int i = 0; i < m_iClusteresNum; i++ )
	{
		pCountList = new int[m_sm_calcs[i]->GetNumVertices()];
		pLayerList = new int[m_sm_calcs[i]->GetNumVertices()];
		
		for(j = 0; j < m_sm_calcs[i]->GetNumVertices(); j++)
		{
			
			pIndx = m_sm_calcs[i]->GetParticleIndx(j);

			//粒子が所属する何番目のクラスタなのかを求める
			pCountList[j] = m_ice->GetPtoCNum_Calc(pIndx)-PtoCNum[pIndx];
			PtoCNum[pIndx]--;

			//粒子が何階層目でクラスタに属したかを求める
			pLayerList[j] = m_sm_calcs[i]->GetLayer(j);
		}

		const vector<int>& pVector = m_sm_calcs[i]->GetVertexIndxList();
		MakeClusterInfo_Calc(i, pVector, pCountList, pLayerList);
...
//	cout << __FUNCTION__ << " set" << endl;
	...
	//接続クラスタと計算クラスタの関連情報の登録
	m_ice->MakeCalcToConnect();				//初期化

	for(int i = 0; i < m_iClusteresNum; i++)
	{
		MakeCalcToConnectInfo(i);
	}

	//RXTIMER("InitICE_Layr Set CalcToConnect");
	//cout << __FUNCTION__ << " Cluster neighbor" << endl;
}
 ...
 * @param
 */
void rxFlGLWindow::StepICE_Melt()
{//	cout << "StepICE_Melt" << endl;	//	RXTIMER("melt0");
	vector<int> viParticleIndxList;										//融解した粒子群

	//探索
	//SM法の削除処理があるので並列化は不可能
	for(int i = 0; i < m_pPS->GetNumParticles(); i++)
	{	
		if( m_fIntrps.size() <= (unsigned)i)				continue;	//存在しない場合は戻る
		if( m_ht->getPhaseChange(i) != 1 )					continue;	//相転移の条件を満たしていない場合は戻る
		if( m_ht->getPhase(i) != 2 )						continue;	//水へと相転移していない場合は戻る
		if( m_ice->GetPtoCNum_Connect(i) == 0 )				continue;	//クラスタに含まれている
		
//		if(viParticleIndxList.size() > 10){	break;	}					//融解粒子数の制限

		int pIndx = i;
		
		m_fIntrps[pIndx] = 0.0f;										//線形補間もしない
		m_ht->setPhaseChange(pIndx, 0);									//相転移し終わったことを伝える
		
		viParticleIndxList.push_back(pIndx);							//融解粒子の記録

		//１接続クラスタの処理　粒子削除
		for(int j = 0; j < m_ice->GetPtoCIndx_Connect(pIndx); j++)
		{
			int* coSet = m_ice->GetPtoC_Connect(pIndx, j);
			int cIndx = coSet[0];
			int oIndx = coSet[1];
			...
			m_ice->DeleteCtoP_Connect(cIndx, oIndx);
			m_ice->ResetOrderPToC(cIndx, oIndx);

			m_sm_connects[cIndx]->Remove(oIndx);
		}

		m_ice->ClearPtoC_Connect(pIndx);
		//m_ice->ClearPtoC_Calc(pIndx);								//こっちは後で使うので，そのまま
	}

//	cout << __FUNCTION__ << "計算クラスタ削除" << endl;
	//RXTIMER("melt1");

	//接続クラスタの粒子数が０　かつ　計算クラスタの粒子数が０でない　クラスタを削除
	for(int i = 0; i < m_sm_calcs.size(); i++)
	{	
		CheckZeroCluster_Connect(i);
	}

//	cout << __FUNCTION__ << "計算クラスタ再定義" << endl;
	//RXTIMER("melt2");

	//２計算クラスタの処理　再定義
	//複数粒子が同一フレーム内に再定義した場合を考慮して，全ての接続クラスタ情報の更新→計算クラスタの更新　としている
	if(viParticleIndxList.size() != 0)
	{
		ReConstructCalcCluster_Melt(viParticleIndxList);
	}

//	cout << __FUNCTION__ << "終了" << endl;
	//RXTIMER("melt3");
 ...
 * @param
 */
void rxFlGLWindow::StepICE_ClusterConstruct()
{
 ...
 * @param
 */
void rxFlGLWindow::StepICE_Freeze()
{//	cout << __FUNCTION__ << endl;
	return;
	vector<int> phaseChangeParticleList;								//相転移可能な粒子のリスト
	bool bFreezeFlag = false;											//計算クラスタの再定義を行うかの判定
	...
	//　　近傍氷塊粒子数で分岐
	//　　　０個　処理終了
	//　　　１個以上２個以下　最近傍氷塊粒子を３個になるまで探索し，クラスタ作成．
	//　　　３個　クラスタ作成．
	//　　　３個以上　最近傍の３個でクラスタ作成．
	//					TODO：：tetgenを利用してきれいな四面体を作成．複数クラスタへ所属するので安定する．
	//					課題：：メッシュ作成が困難．
	//					進捗：：５点ならば四面体が２つできるらしい．（確定ではない）
	bFreezeFlag = StepICE_Freeze_MakeCluster(phaseChangeParticleList);

//	cout << __FUNCTION__ << " Check1" << endl;

	//２　氷塊クラスタ用処理
	//　B 相転移粒子の近傍氷塊の含有粒子数で分岐
	//　　　０個　氷塊粒子ではない
	//　　　１個以上３個以下　相転移粒子を追加
	//　　　４個　処理終了
	//近傍氷塊粒子を調べ，相転移可能粒子を氷塊粒子数４個未満のクラスタに追加
//	bFreezeFlag = StepICE_Freeze_AddParticle(phaseChangeParticleList);

//	cout << __FUNCTION__ << " Check2" << endl;

	//３　相転移可能粒子のみでの凝固
//	bFreezeFlag = StepICE_Freeze_PhaseChangeParticles(phaseChangeParticleList);

//	cout << __FUNCTION__ << " Check3" << endl;

	//４　計算クラスタの再定義
	if(!bFreezeFlag) return;
//	ReConstructCalcCluster_Freeze(phaseChangeParticleList);				//計算クラスタの再定義
...
/*!
 * 相転移可能粒子と近傍氷塊での凝固
 * @param[in] pList　相転移可能粒子のリスト　使いまわすので参照渡し．
 * @param[out] bool　クラスタが作成されたらtrue　されていないならfalse
 */
bool rxFlGLWindow::StepICE_Freeze_MakeCluster(vector<int>& pList)
{//	cout << "StepICE_Freeze_MakeCluster" << endl;
	...
	for( int i = 0; i < m_pPS->GetNumParticles(); i++ )
	{
		//１　資格判定　中心粒子の条件を満たさないなら戻る
		if( m_ht->getPhaseChange(i) != 1 )				continue;			//相転移条件を満たす
		if( neights[i].size() <= 1 )					continue;			//近傍粒子が存在する　近傍粒子は自分を含むから１
		if( m_ice->GetPtoCNum_Connect(i) > 0 )			continue;			//クラスタに所属しているなら戻る

		vector<int> vParticleIndxes;										//クラスタを作成する粒子の集合

		int pIndx = i;														//相転移粒子の番号
		pList.push_back(pIndx);												//相転移粒子の集合
//		cout << __FUNCTION__ << " check1 pIndx = " << pIndx << endl;

		//２　探索処理　近傍氷塊粒子を調べる
		for( unsigned j = 0; j < neights[pIndx].size(); j++ )
		{
			//近傍粒子の条件を満たさないなら戻る
			int jpIndx = neights[pIndx][j].Idx;											//近傍粒子の番号
			
			if( pIndx == jpIndx )											continue;	//自分自身は除く
			if( m_ht->getTemps()[jpIndx] > 250 )							continue;	//粒子の温度が一定以下
			if( m_ice->GetPtoCNum_Connect(jpIndx) <= 0 )					continue;	//クラスタのどれかに所属していないなら戻る
			
			float distance	= neights[pIndx][j].Dist2;
			float radius	= pow(((RXSPH*)m_pPS)->GetEffectiveRadius()*0.5f, 2); 

			if(	distance > radius )											continue;	//凝固粒子との距離が影響半径の半分
			vector<int>::iterator check = std::find( vParticleIndxes.begin(),
													 vParticleIndxes.end()  ,
													 jpIndx				);
			if( check != vParticleIndxes.end() )							continue;	//すでに含んでいたら戻る

			vParticleIndxes.push_back(jpIndx);											//粒子番号を登録
		}

		//３　近傍氷塊粒子数で分岐　終了か最探索
		int iParticleSize = vParticleIndxes.size();
		if( iParticleSize == 0 )
		{
			//処理終了
//			cout << __FUNCTION__ << " check3a iParticleSize = " << iParticleSize << endl;
			bFreezeFlag = false;	continue;
		}
		else if( 1 <= iParticleSize && iParticleSize <= 2 )
		{
			//最近傍氷塊粒子が３個になるまで探索　近傍クラスタ0層目まで探索
//			cout << __FUNCTION__ << " check3b iParticleSize = " << iParticleSize << endl;

			for( unsigned j = 0; j < vParticleIndxes.size(); j++ )
			{
				int pIndx = vParticleIndxes[j];
				vector<int> pList = GetNeighborDistanceIceParticles(pIndx, 0, 20);			//最大20個取得
				if( pList.size() > 4 ){	pList.erase( pList.begin()+4, pList.end() );	}	//4個にカット　なぜ最大４個で取得しないかというと，４個見つかった時点で探索をやめてしまうため．精度を上げたい．もっとうまく書けるだろ！
				...
				for( unsigned k = 0; k < pList.size(); k++ )
				{
					...
					vector<int>::iterator check = std::find( vParticleIndxes.begin(),
															 ...
															 kpIndx				);
					if( check != vParticleIndxes.end() ){	continue;	}					//すでに含んでいたら戻る
					vParticleIndxes.push_back(kpIndx);

					if( vParticleIndxes.size() == 3 ){	break;	}
				}
				if( vParticleIndxes.size() == 3 ){	break;	}
			}
		}
		...
		{
//			cout << __FUNCTION__ << " check3c iParticleSize = " << iParticleSize << endl;
		}

		vParticleIndxes.push_back(pIndx);
		iParticleSize = vParticleIndxes.size();

		//デバッグ
		//for( unsigned j = 0; j < vParticleIndxes.size(); j++ )
		//{
		//	cout << "j = " << j << " pIndx = " << vParticleIndxes[j] << endl;
		//}

		//４　クラスタ作成　相転移粒子＋氷塊粒子
		if( iParticleSize >= 4 )
		{
//			cout << __FUNCTION__ << " check4a iParticleSize = " << iParticleSize << endl;
			//試作　複数の四面体を考えない
			if( vParticleIndxes.size() > 4 )
			{
				vParticleIndxes.erase( vParticleIndxes.begin()+3, vParticleIndxes.end()-1 );
			}

			//クラスタ作成，各種情報登録
			MakeCluster(vParticleIndxes);								//接続クラスタ，計算クラスタ（初期）の作成
			MakeClusterInfo_Connect(m_iClusteresNum-1);					//接続クラスタの情報登録
			m_ice->SetNeighborCluster(m_iClusteresNum-1, m_iLayer);		//近傍クラスタの情報登録
			MakeClusterCalc(m_iClusteresNum-1);							//計算クラスタ（完成）の作成
			MakeClusterInfo_Calc(m_iClusteresNum-1);					//計算クラスタの情報登録
			MakeCalcToConnectInfo(m_iClusteresNum-1);					//計算クラスタ→接続クラスタの情報登録

			m_fIntrps[pIndx] = 1.0f;									//線形補間もしない
			m_ht->setPhaseChange(pIndx, 0);								//相転移し終わったことを伝える
			bFreezeFlag = true;
		}
		else
		{
//			cout << __FUNCTION__ << " check4b iParticleSize = " << iParticleSize << endl;
			bFreezeFlag = false;
		}
	}
...
/*!
 * 相転移可能粒子を氷塊粒子数４個未満のクラスタに追加
 * @param[in] pList	 相転移粒子群
 */
bool rxFlGLWindow::StepICE_Freeze_AddParticle(vector<int>& pList)
{//	cout << __FUNCTION__ << endl;
	...
	{
		int ipIndx = pList[i];
		if( m_ht->getPhaseChange(ipIndx) != 1 )							continue;			//相転移条件を満たす
		...
		for( unsigned j = 0; j < neights[ipIndx].size(); j++ )
		{
			...
			float radius	= pow(((RXSPH*)m_pPS)->GetEffectiveRadius()*0.75f, 2); 

			if(	distance > radius )										continue;		//凝固粒子との距離が影響半径の半分

			//２　追加可能な氷塊を見つける
			for(int k = 0; k < m_ice->GetPtoCIndx_Connect(jpIndx); k++)
			{
				int* coSet = m_ice->GetPtoC_Connect(jpIndx, k);
				int kcIndx = coSet[0];

				if(kcIndx == -1)								{		continue;	}
				if(m_sm_connects[kcIndx]->GetNumVertices() >= 4){		continue;	}	//氷塊粒子の含有粒子数が４以上の場合は戻る
				if(m_sm_connects[kcIndx]->CheckIndx(ipIndx))	{		continue;	}

				AddVertexToCluster(ipIndx, kcIndx);										//クラスタへ粒子の追加
...
/*!
 * 相転移可能粒子群のみでの凝固
 * @param[in] pList　相転移可能粒子のリスト
 */
bool rxFlGLWindow::StepICE_Freeze_PhaseChangeParticles(vector<int>& pList)
{//	cout << "StepICE_Freeze_PhaseChangeParticle" << endl;
	vector<vector<rxNeigh>>& neights = ((RXSPH*)m_pPS)->GetNeights();		//近傍粒子を取得
	RXREAL *p = m_pPS->GetArrayVBO(rxParticleSystemBase::RX_POSITION);
	bool bAddFlag = false;
	...
	for(unsigned i = 0; i < pList.size(); i++)
	{
		...
		if( neights[i].size() <= 1 ){			continue;	}				//近傍粒子が存在　近傍粒子は自分を含むから１
		
		vector<int> pcpList;												//近傍粒子をたどった相転移可能粒子の集合
		pcpList.push_back(pIndx);

		//近傍粒子をたどり，相転移可能粒子の集合を得る
		for(unsigned j = 0; j < pcpList.size(); j++)
		{
			int jpIndx = pcpList[j];
			if( neights[jpIndx].size() <= 1 ){	continue;	}				//近傍粒子が存在　近傍粒子は自分を含むから１
			
			//近傍相転移可能粒子を追加
			for(unsigned k = 0; k < neights[jpIndx].size(); k++)
			{
				int npIndx = neights[jpIndx][k].Idx;
				if( m_ht->getPhaseChange(npIndx) != 1 ){	continue;	}	//相転移条件を満たす

				float distance	= neights[jpIndx][k].Dist2;
				float radius	= pow(((RXSPH*)m_pPS)->GetEffectiveRadius()*0.75f, 2); 
				//cout << "distance = " << distance << endl;
				//cout << "radius  =  " << radius << endl;
				if(	distance > radius )					{	continue;	}	//凝固粒子との距離が影響半径の半分
				if( std::find(pcpList.begin(), pcpList.end(), npIndx) != pcpList.end() ){	continue;	}			//すでに含んでいたら戻る
				
				pcpList.push_back(npIndx);
			}
		}

		//クラスタ作成
		if( pcpList.size() >= 4 )									//4個以上の場合　デバッグしてない
		{
			//	cout << "pcpList.size() >= 4pcpList.size() >= 4" << endl;
			if( pcpList.size() > 4 ){ pcpList.erase( pcpList.begin()+4, pcpList.end() );	}	//試作　複数の四面体を考えない
		}
		else if(pcpList.size() <= 1 )
		{
			continue;
		}

		//クラスタ作成，各種情報登録
		MakeCluster(pcpList);										//接続クラスタ，計算クラスタ（初期）の作成
		MakeClusterInfo_Connect(m_iClusteresNum-1);					//接続クラスタの情報登録
		m_ice->SetNeighborCluster(m_iClusteresNum-1, m_iLayer);		//近傍クラスタの情報登録
		MakeClusterCalc(m_iClusteresNum-1);							//計算クラスタ（完成）の作成
		MakeClusterInfo_Calc(m_iClusteresNum-1);					//計算クラスタの情報登録
		...
		{
			int jpIndx = pcpList[j];
			m_fIntrps[jpIndx] = 1.0f;										//線形補間もしない
			m_ht->setPhaseChange(jpIndx, 0);								//相転移し終わったことを伝える
		}
...
/*!
 * 融解時の計算クラスタ再定義
 * 融解粒子を含んでいる計算クラスタを探索し，そのペアとなる接続クラスタを得る
 * 接続クラスタの近傍クラスタを更新し，計算クラスタを再定義する
 * @param[in] pIndxList　融解粒子群
 */
void rxFlGLWindow::ReConstructCalcCluster_Melt(vector<int>& pIndxList)
{//	cout << __FUNCTION__ << " pIndxList.size()" << pIndxList.size() << endl;

	//ある粒子の所属する計算クラスタの集合から，接続クラスタの集合を得る
	vector<int> viCalcClusterList;									//再定義するクラスタの集合
	vector<int> viCalcClusterLayerList;								//再定義するクラスタのレイヤー

	int j= 0, k = 0;
	int  pIndx = 0, cIndx = 0;
	int jpIndx = 0,jcIndx = 0;
	int kpIndx = 0,kcIndx = 0,koIndx = 0;
	int jlayer = -1;
	...
	//再定義するクラスタの探索，クラスタを何階層目から再定義するかの探索
	//複数粒子が存在する場合，低いレイヤーが優先されるよう上書きする
	for(int i = 0; i < pIndxList.size(); i++)
	{
		int pIndx = pIndxList[i];

		for(int j = 0; j < m_ice->GetPtoCIndx_Calc(pIndx); j++)
		{
			int* coSet = m_ice->GetPtoC_Calc(pIndx, j);
			if(coSet[0] == -1 || coSet[1] == -1){				continue;	}

			vector<int>::iterator check = std::find(viCalcClusterList.begin(),
													viCalcClusterList.end()  ,
													coSet[0]
													);

			//既に含んでいたら，layerの代償を比べて低いほうを優先する．
			if( check != viCalcClusterList.end() )
			{
				int layerIndx = check-viCalcClusterList.begin();
				if(viCalcClusterLayerList[layerIndx] > coSet[2])
				{
					viCalcClusterLayerList[layerIndx] = coSet[2];
				}
				continue;
			}

			viCalcClusterList.push_back(coSet[0]);
			viCalcClusterLayerList.push_back(coSet[2]);
		}
	}
...
#pragma omp for
	for(int i = 0; i < pIndxList.size(); i++)
	{
		m_ice->ClearPtoC_Calc(pIndxList[i]);
	}
}//end #pragma omp parallel

//	cout << __FUNCTION__ << " Check1a viCalcClusterList.size() = " << viCalcClusterList.size() << endl;
	
	//接続クラスタで内容が同一，または包含関係にあるものを削除．
	//CheckSameCluster_Connectを並列化するしかない
//	for(int i = 0; i < viCalcClusterList.size(); i++)
//	{
//		int cIndx = viCalcClusterList[i];
////		if(m_sm_connects[cIndx]->GetNumVertices() == 4){	continue;	}	//粒子数４で被りは（融解だけだと）ありえない
//		
//		vector<int> icIndxList = CheckSameCluster_Connect(cIndx);			//内容が同一の接続クラスタが存在するか
//
//		//同一内容のクラスタを，再定義するクラスタから削除
//		for(int j = 0; j < icIndxList.size(); j++)
//		{	
//			int jcIndx = icIndxList[j];		
//			vector<int>::iterator end_it = std::remove( viCalcClusterList.begin(), viCalcClusterList.end(), jcIndx );	//removeは厳密な削除で無いのに注意
//			viCalcClusterList.erase( end_it, viCalcClusterList.end() );													// 本当の意味での削除を実行する
//		}
//	}

//	cout << __FUNCTION__ << " Check1b viCalcClusterList.size() = " << viCalcClusterList.size() << endl;
	//RXTIMER("check2");

	//再定義する計算クラスタに含まれる，粒子の情報を更新
	//粒子の持つ，再定義する前の計算クラスタについての情報を取り除く
	//並列化するとたまに挙動が怪しい　影響大
#pragma omp parallel
{
#pragma omp for private(cIndx,j,jpIndx,k,coSet)
	for(int i = 0; i < viCalcClusterList.size(); i++)
	{
		cIndx = viCalcClusterList[i];

		for(j = 0; j < m_ice->GetCtoPIndx_Calc(cIndx); j++)
		{
			jpIndx = m_ice->GetCtoP_Calc(cIndx, j)[0];					//再定義前の計算クラスタが含んでいる粒子を取得
			if(jpIndx == -1){	continue;	}

			for(k = 0; k < m_ice->GetPtoCIndx_Calc(jpIndx); k++)
			{
				coSet = m_ice->GetPtoC_Calc(jpIndx, k);
				
				if(coSet[0] != cIndx)				{	continue;	}
				if(coSet[0] == -1 || coSet[1] == -1){	continue;	}

//#pragma omp critical (DeletePtoC_Calc)
{
				m_ice->DeletePtoC_Calc(jpIndx, k);
}
				break;			//同じクラスタに複数所属することは無いので，break
			}
		}
...
//	cout << __FUNCTION__ << " Check2 " << endl;
	...
	//再定義する計算クラスタの初期化，近傍クラスタの再定義
	//並列化は限界．SetNeighborClusterのアルゴリズムをいじるしかない　影響最大
#pragma omp parallel
{
#pragma omp for private(cIndx,j,jlayer,plSet)

	for(int i = 0; i < viCalcClusterList.size(); i++)
	{
		cIndx = viCalcClusterList[i];

		//cIndx接続クラスタが，融解した粒子を含んでいたかどうかで分岐
		//layer0で粒子を含んでいるなら，普通の再定義
		//layer0で粒子を含んでいないなら，layerを考慮した再定義
		//ここが結構重いみたい
		jlayer = -1;
		for(j = 0; j < m_ice->GetCtoPNum_Calc(cIndx); j++)
		{
			plSet = m_ice->GetCtoP_Calc(cIndx, j);
			if(plSet[0] == -1 || plSet[1] == -1){	continue;	}

			//融解粒子を含んでいるなら，layer番号を取得
			if(std::find(pIndxList.begin(),pIndxList.end(),plSet[0]) == pIndxList.end()){	continue;	}

			jlayer = plSet[1];
			break;
		}

		m_ice->ClearCalcToConnect(cIndx);									//計算クラスタ→接続クラスタの情報をクリア
		m_ice->ClearCtoP_Calc(cIndx);										//クラスタ→粒子の情報をある階層までクリア


		//近傍クラスタの再定義
		if(jlayer == 0)
		{
			m_ice->ClearNeighborCluster(cIndx);								//近傍クラスタの情報をクリア
			m_ice->SetNeighborCluster(cIndx, m_iLayer);						//近傍クラスタの再定義
		}
		else if(jlayer != -1)
		{
			m_ice->SetNeighborClusterFromCluster(cIndx, m_iLayer, jlayer);	//layerを考慮した近傍クラスタの再定義
//			m_ice->ClearNeighborCluster(cIndx);								//近傍クラスタの情報をクリア
//			m_ice->SetNeighborCluster(cIndx, m_iLayer);						//近傍クラスタの再定義
		}
		else
		{
			cout << "Error::jlayer == -1 cIndx = " << cIndx << endl;
			m_ice->ClearNeighborCluster(cIndx);								//近傍クラスタの情報をクリア
			m_ice->SetNeighborCluster(cIndx, m_iLayer);						//近傍クラスタの再定義
		}
	}
...
//	cout << __FUNCTION__ << " Check3" << endl;
	...
	//SM法の計算クラスタの再定義
	//影響ほぼなし
	for( unsigned i = 0; i < viCalcClusterList.size(); i++ )
	{
		int cIndx = viCalcClusterList[i];
		//計算クラスタに含まれる全ての粒子を削除し，粒子を入れなおす．
		m_sm_calcs[cIndx]->Clear();

		for( int j = 0; j < m_sm_connects[cIndx]->GetNumVertices(); j++ )
		{
			m_sm_calcs[cIndx]->AddVertex( m_sm_connects[cIndx]->GetVertexPos(j), 1.0, m_sm_connects[cIndx]->GetParticleIndx(j) );
			m_sm_calcs[cIndx]->SetAlphas(j, 1.0);
			m_sm_calcs[cIndx]->SetBetas (j, 0.0);
			m_sm_calcs[cIndx]->SetLayer (j, 0);
		}
	}
...
//	cout << __FUNCTION__ << " Check4" << endl;
	...
	//SM法の近傍クラスタを探索し，周囲の粒子を追加する
	//影響大
#pragma omp parallel
{
#pragma omp for private(cIndx,j,jcIndx,k,kpIndx)
	for(int i = 0; i < viCalcClusterList.size(); i++)
	{
		cIndx = viCalcClusterList[i];

		for(j = 0; j < m_ice->GetNCNum(cIndx); j++)
		{
			jcIndx = m_ice->GetNeighborCluster(cIndx, j)[0];
			layer = m_ice->GetNeighborCluster(cIndx, j)[1];

			for(k = 0; k < m_ice->GetCtoPIndx_Connect(jcIndx); k++)
			{
				kpIndx = m_ice->GetCtoP_Connect(jcIndx, k);

				if(kpIndx == -1){							continue;	}
				if(m_sm_calcs[cIndx]->CheckIndx(kpIndx)){	continue;}
				
				m_sm_calcs[cIndx]->AddVertex( Vec3( p[kpIndx*4+0], p[kpIndx*4+1], p[kpIndx*4+2] ), 1.0, kpIndx);
				m_sm_calcs[cIndx]->SetAlphas(m_sm_calcs[cIndx]->GetNumVertices()-1, 1.0);
				m_sm_calcs[cIndx]->SetBetas (m_sm_calcs[cIndx]->GetNumVertices()-1, 1.0);
				m_sm_calcs[cIndx]->SetLayer (m_sm_calcs[cIndx]->GetNumVertices()-1, layer);
			}
...
//	cout << __FUNCTION__ << " Check5" << endl;
	...
	//氷構造の計算クラスタの登録
	//カウントなので並列化はあやしい？	ほぼ影響なし
	for(unsigned i = 0; i < viCalcClusterList.size(); i++)
	{
		int cIndx = viCalcClusterList[i];
		for(int j = 0; j < m_sm_calcs[cIndx]->GetNumVertices(); j++)
		{
			int jpIndx = m_sm_calcs[cIndx]->GetParticleIndx(j);
			m_ice->CountPtoC_Calc(jpIndx);
			m_ice->CountCtoP_Calc(cIndx);								//計算クラスタが粒子を含む個数のカウント
		}
		m_ice->SetCtoPIndx_Calc(cIndx, m_ice->GetCtoPNum_Calc(cIndx));	//Indxを初期化
	}

//	cout << __FUNCTION__ << " Check5a" << endl;
	//RXTIMER("check6a");
	
	//データ登録　粒子→クラスタ　クラスタ→粒子
	//完全な並列化は難しい　悪影響がでるかも？　影響大
	int* pPIndxList;
	int* pLayerList;
	vector<int> pVector;
...
#pragma omp parallel
{
#pragma omp for private(pPIndxList,pLayerList,pVector,coSet,cIndx,pIndx,j,k)
	for(int i = 0; i < viCalcClusterList.size(); i++)
	{
		cIndx = viCalcClusterList[i];
		pPIndxList = new int[m_sm_calcs[cIndx]->GetNumVertices()];
		pLayerList = new int[m_sm_calcs[cIndx]->GetNumVertices()];

		//PtoC_Calcの配列で空き場所を探索
		for(j = 0; j < m_sm_calcs[cIndx]->GetNumVertices(); j++)
		{
			pIndx = m_sm_calcs[cIndx]->GetParticleIndx(j);

			//初期化と違い，更新する配列のIndxを指定しないといけない．
			//B　-1を探索＋上書きタイプ
			for(k = 0; k < m_pPS->GetMaxParticles()/5/2; k++)
			{
				coSet = m_ice->GetPtoC_Calc(pIndx, k);

				if(coSet[0] != -1 || coSet[1] != -1){	continue;	}

#pragma omp critical (SetPtoCIndx_Calc)
{
				if(k >= m_ice->GetPtoCIndx_Calc(pIndx))
				{
					m_ice->SetPtoCIndx_Calc(pIndx, k+1);					//現在のIndxより大きいなら更新
				}
}
				pPIndxList[j] = k;
				pLayerList[j] = m_sm_calcs[cIndx]->GetLayer(j);				//別にここでなくともいい

				break;
			}
		}

		pVector = m_sm_calcs[cIndx]->GetVertexIndxList();
		MakeClusterInfo_Calc(cIndx, pVector, pPIndxList, pLayerList);		//データ登録
...
//	cout << __FUNCTION__ << " Check5b" << endl;
	//RXTIMER("check6b");

	//計算クラスタ→接続クラスタの初期情報を登録
#pragma omp parallel
{
#pragma omp for private(cIndx,j)
	for(int i = 0; i < viCalcClusterList.size(); i++)
	{
		cIndx = viCalcClusterList[i];
		for(j = 0; j < m_ice->GetCtoPNum_Connect(cIndx); j++)
		{
			m_ice->SetCalcToConnect(cIndx, j, cIndx, j);		//登録
		}
	}
}//end #pragma omp parallel
...
//	cout << __FUNCTION__ << " Check6" << endl;
	...
	//計算クラスタ→接続クラスタの情報を，近傍クラスタを基に登録
	//SM法の登録時と同じ手順を踏むことで行う．
	//影響中
	int ioIndx = 0;
	int kcooIndx = 0;	//COnnect Ordr Indx

#pragma omp parallel
{
#pragma omp for private(cIndx,j,jcIndx,k,kpIndx,ioIndx,kcooIndx)
	for(int i = 0; i < viCalcClusterList.size(); i++)
	{
		cIndx = viCalcClusterList[i];
		ioIndx = m_ice->GetCtoPNum_Connect(cIndx);										//添え字は，粒子数から開始
		...
		for(j = 0; j < m_ice->GetNCNum(cIndx); j++)
		{
			...
			for(k = 0; k < m_ice->GetCtoPIndx_Connect(jcIndx); k++)
			{
				...
				kcooIndx++;
				if(m_sm_calcs[cIndx]->GetParticleIndx(ioIndx) != kpIndx){ continue;	}
				m_ice->SetCalcToConnect(cIndx, ioIndx, jcIndx, kcooIndx);				//登録
				ioIndx++;
			}
		}
	}
	...
	m_iShowClusterIndx = m_sm_connects.size();
...
/*!
 * 凝固時の計算クラスタ再定義
 * 凝固粒子を含んでいる計算クラスタを探索し，そのペアとなる接続クラスタを得る
 * 接続クラスタの近傍クラスタを更新し，計算クラスタを再定義する
 * @param[in] pIndxList　凝固粒子群
 */
void rxFlGLWindow::ReConstructCalcCluster_Freeze(vector<int>& pIndxList)
{	cout << __FUNCTION__ << " pIndxList.size()" << pIndxList.size() << endl;
...
/*!
 * 同一の内容＋包含している接続クラスタを削除する　融解専用
 */
vector<int> rxFlGLWindow::CheckSameCluster_Connect(int cIndx)
{//	cout << __FUNCTION__ << endl;

	Ice_SM* connect = m_sm_connects[cIndx];
	vector<int> cIndxList;										//削除するクラスタ

	//同一内容の探索
	if(connect->GetNumVertices() != 4)
	{
		for(int i = 0; i < m_iClusteresNum; i++)
		{
			if(cIndx == i){															continue;	}
			if(connect->GetNumVertices() != m_sm_connects[i]->GetNumVertices() ){	continue;	}
			
			//内容が完全に一致しているかのチェック
			bool check = false;
			for(int j = 0; j < connect->GetNumVertices(); j++)
			{
				//j番目の粒子が共通するかをチェック
				check = false;

				for(int k = 0; k < m_sm_connects[i]->GetNumVertices(); k++)
				{
					if(connect->GetParticleIndx(j) == m_sm_connects[i]->GetParticleIndx(k))
					{		check = true; break;	}
					else{	check = false;			}
				}

				if(!check){		break;			}					//1つでも含んでいないなら一致しない
			}

			if(check)
			{
				cIndxList.push_back(i);								//最後までtrueなら一致．
			}
		}
	}
	...
	//		for(int k = 0; k < connect->GetNumVertices(); k++)
	//		{
	//			if(m_sm_connects[i]->GetParticleIndx(j) == connect->GetParticleIndx(k))
	//			{		check = true; break;	}
	//			else{	check = false;			}
	//		}
	...
	if(cIndxList.size() == 0){	return cIndxList;	}			//空のベクタを返す

	//クラスタ情報の削除　m_iceの内容を更新
	for(int i = 0; i < cIndxList.size(); i++)
	{
		int icIndx = cIndxList[i];
		DeleteIceInfoCluster(icIndx);
	}

	return cIndxList;											//ローカル変数なので参照では返せない
}

/*!
 * 	接続クラスタの粒子数が０　かつ　計算クラスタの粒子数が０でない　クラスタを削除
 */
void rxFlGLWindow::CheckZeroCluster_Connect(int cIndx)
{
	if(m_sm_connects[cIndx]->GetNumVertices() != 0) return;		//接続クラスタの粒子数が0
	if(m_sm_calcs[cIndx]->GetNumVertices() == 0)	return;		//計算クラスタの粒子数が0でない

	//粒子の情報を更新
	for(int j = 0; j < m_ice->GetCtoPIndx_Calc(cIndx); j++)
	{
		int jpIndx = m_ice->GetCtoP_Calc(cIndx, j)[0];
		if(jpIndx == -1){	continue;	}

		for(int k = 0; k < m_ice->GetPtoCIndx_Calc(jpIndx); k++)
		{
			int* coSet = m_ice->GetPtoC_Calc(jpIndx, k);
			if(coSet[0] == -1 || coSet[1] == -1){	continue;	}
			if(coSet[0] != cIndx)				{	continue;	}
			m_ice->DeletePtoC_Calc(jpIndx, k);	break;
		}
	}
	...
	m_sm_calcs[cIndx]->Clear();
	m_ice->ClearCalcToConnect(cIndx);								//計算クラスタ→接続クラスタの情報をクリア
	m_ice->ClearCtoP_Calc(cIndx);									//計算クラスタ→粒子の情報をクリア
	m_ice->ClearNeighborCluster(cIndx);								//近傍クラスタの情報をクリア
}
 ...
 * 粒子情報の削除
 */
void rxFlGLWindow::DeleteIceInfoParticle(int pIndx)
{
 ...
 * クラスタ情報の削除
 */
void rxFlGLWindow::DeleteIceInfoCluster(int cIndx)
{//	cout << __FUNCTION__ << endl;
	//SM法　接続クラスタ
	m_sm_connects[cIndx]->Clear();

	//SM法　計算クラスタ
	m_sm_calcs[cIndx]->Clear();

	//氷塊構造　粒子→接続クラスタ
	for(int i = 0; i < m_ice->GetCtoPIndx_Connect(cIndx); i++)
	{
		int ipIndx = m_ice->GetCtoP_Connect(cIndx, i);
		if(ipIndx == -1){	continue;	}

		for(int j = 0; j < m_ice->GetPtoCIndx_Connect(ipIndx); j++)
		{
			int* coSet = m_ice->GetPtoC_Connect(ipIndx, j);
			if(coSet[0] == -1 || coSet[1] == -1){	continue;	}
			if(coSet[0] != cIndx)				{	continue;	}

			m_ice->DeletePtoC_Connect(ipIndx, j);	break;
		}
	}

	//氷塊構造　粒子→計算クラスタ
	for(int i = 0; i < m_ice->GetCtoPIndx_Calc(cIndx); i++)
	{
		int ipIndx = m_ice->GetCtoP_Calc(cIndx, i)[0];
		if(ipIndx == -1){	continue;	}

		for(int j = 0; j < m_ice->GetPtoCIndx_Calc(ipIndx); j++)
		{
			int* coSet = m_ice->GetPtoC_Calc(ipIndx, j);
			if(coSet[0] == -1 || coSet[1] == -1){	continue;	}
			if(coSet[0] != cIndx)				{	continue;	}

			m_ice->DeletePtoC_Calc(ipIndx, j);	break;
		}
	}

	//氷塊構造　接続クラスタ
	m_ice->ClearCtoP_Connect(cIndx);								//接続クラスタ→粒子の情報をクリア
...
/*!
 * 計算クラスタの再定義　試作　全クラスタ版　バグあり．
 */
void rxFlGLWindow::ReConstructCalcCluster()
{
}
 ...
 * @param[in] dt タイムステップ幅
 */
void rxFlGLWindow::StepICE_CalcParametor(double dt)
{//	cout << "StepICE_CalcParametor" << endl;
	if(m_bPause) return;
//	cout << __FUNCTION__ << "Step1" << endl;
	//熱処理により変動した温度を,各パラメータに適用
	for( int i = 0; i < m_pPS->GetNumParticles(); i++ )
 ...
 * @param[in] dt タイムステップ幅
 */
void rxFlGLWindow::StepICE_Interpolation(double dt)
{//	cout << "StepICE_Interpolation" << endl;
	return;
	if(!m_bPause)
	{//	cout << "StepICE_Interpolation" << endl;
		RXREAL *p = m_pPS->GetArrayVBO(rxParticleSystemBase::RX_POSITION);
		...
		for( int i = 0; i < m_pPS->GetNumParticles(); i++ )
		{
			...
			for( int j = 0; j < m_ice->GetPtoCIndx_Connect(i); j++ )
			{
				...
				int oIndx = coSet[1];
				if(cIndx == -1 || oIndx == -1) continue;
				pos += m_sm_calcs[cIndx]->GetVertexPos(oIndx);
				vel += m_sm_calcs[cIndx]->GetVertexVel(oIndx);
				shapeNum++;
			}
